...
function redo_tags(c) {
    var a = c.document.getElementsByTagName("a");
    for (var d = 0; d < a.length; d++) {
        var b = a[d];
        if (b.href != "#") {
            b.href = mopub_click_url + "&r=" + b.href;
        }
    }
}
...
function mp_cb(a) {
    ufid = a.ufid;
    mopub_click_url = a.click_url;
    if (device.platform == "native") {
        mopub_click_url = "external://" + a.click_url;
    }
    var b = document.getElementById("mopub-iframe-" + a.ufid);
    if (!b) {
        return;
    }
    if (a.ad) {
        b = (b.contentWindow) ? b.contentWindow : (b.contentDocument.document);
        b.document.open();
        b.document.write(a.ad);
        redo_tags(b);
        b.document.close();
    } else {
...
function showMoPubAdvertisementIn(g) {
    ...
    if (true) {
        var l = '<script type="text/javascript"> var mopub_ad_unit="' + mopub_ad_unit + '";var mopub_ad_width="' + mopub_ad_width + '";var mopub_ad_height="' + mopub_ad_height + '";</script><script src="http://ads.mopub.com/js/client/mopub.js"></script>';
        var f = $('<iframe id="mopub-iframe" frameborder="0" hspace="0" marginheight="0" marginwidth="0" scrolling="no" vspace="0" style="max-height:50px;" src="about:blank"></iframe>');
        g.append(f);
        var e = f.get(0);
        setTimeout(function () {
    ...
    function b() {
        var q = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz012345678";
        var s = "";
        for (var p = 0; p < 20; p++) {
            s += q.charAt(Math.floor(Math.random() * q.length));
        }
        return s;
    }
    var a = "mopub-udid-cookie";
    if (window.mopub_ad_unit == null) {
        console.log("MoPub load failed. mopub_ad_unit needs to be defined.");
        return;
    }
    var k = document.createElement("a");
    k.href = "http://ads.mopub.com/js/client/mopub.js";
    var j = "http://" + k.hostname;
    if (k.port != "0") {
        j += ":" + k.port;
    }
    var o = b();
    var h = j + "/m/ad?id=" + mopub_ad_unit + "&udid=MOBILEWEBCOOKIE:" + d() + "&ufid=" + o;
    if (window.mopub_keywords != null) {
        h += "&q=" + escape(window.mopub_keywords);
    }
    h += "&jsonp=1&callback=mp_cb";
    g.append('<script type="text/javascript" src="' + j + '/js/clicktracker.js?v=23"></script>');
    g.append('<iframe id="mopub-iframe-' + o + '" frameborder="0" hspace="0" marginheight="0" marginwidth="0" scrolling="no" vspace="0" width="' + window.mopub_ad_width + '" height="' + window.mopub_ad_height + '" src="about:blank"></iframe>');
    g.append('<script type="text/javascript" src="' + h + '"></script>');

    function n(t, w, s, u, q, v) {
        var p = t + "=" + escape(w) + ((s) ? ";expires=" + s.toGMTString() : "") + ((u) ? ";path=" + u : "") + ((q) ? ";domain=" + q : "") + ((v) ? ";secure" : "");
        document.cookie = p;
    }
    function c(s) {
        var t = document.cookie.indexOf(s + "=");
        var q = t + s.length + 1;
        if ((!t) && (s != document.cookie.substring(0, s.length))) {
            return null;
        }
        if (t == -1) {
            return null;
